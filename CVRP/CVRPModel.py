...
class CVRPModel(nn.Module):
    ...
    def pre_forward(self, reset_state, x_edges, x_edges_values, x_node_indices, x_nodes_false):
        ...
        if self.method == "edge_node" or self.method == "KNN_edge":
            ...
            self.node_feature = self.node_encodingblock(x_edges_values, depot_node_demand[:, :, None])  
            # shape：(batch, node+1, embedding)

        self.encoded_nodes = self.node_encoder(depot_xy, node_xy_demand, x_nodes_false, self.node_feature)
        self.encoded_graph = self.graph_encoder(depot_xy, node_xy_demand, x_edges, x_edges_values, x_node_indices, x_nodes_false, self.node_feature) 
        # shape: (batch, problem+1, EMBEDDING_DIM)   
...
class CVRP_GraphEncoder(nn.Module):
    ...
    def forward(self, depot_xy, node_xy_demand, x_edges, x_edges_values, x_node_indices, x_nodes_false, node_feature):
        # [1]node-embedding
        if self.method == "edge_node":   # 1. true node
            ...
            x = torch.cat((embedded_depot, embedded_node), dim=1)  # [B,N+1,H]
        elif self.method == "KNN_edge":  # 2. false node
            x = self.KNN_embedding(x_nodes_false)
        elif self.method == "AB_edge":  # 3. one-hot->emb
            x = node_feature
        elif self.method == "Gknn_Nab":  # 4. knn-false node
            x = self.KNN_embedding(x_nodes_false)
        elif self.method == "Gab_Nknn":  # 5. one-hot->emb
            x = node_feature
        else:
            raise NotImplementedError("Unknown search method")

        # [2]edge-embedding
        e_vals = self.edges_values_embedding(x_edges_values.unsqueeze(3))  # [B,N,N,H]
        e_tags = self.edges_embedding(x_edges)  # [B,N,N,H]
        e = torch.cat((e_vals, e_tags), dim=3)  # [B N+1 N+1 H]
        # GCN layers
        for layer in range(self.num_layers):
            x, e = self.gcn_layers[layer](x, e)  # [B,N,H]  [B,N,N,H]
        # MLP classifier
        graph_encoded = self.mlp_nodes(x)   # [B,N,H]
...
class CVRP_NodeEncoder(nn.Module):
    def __init__(self, **model_params):
        ...
        self.KNN_embedding = nn.Linear(self.knn_node_edge, embedding_dim)   

        self.layers = nn.ModuleList([EncoderLayer(**model_params) for _ in range(encoder_layer_num)])

    def forward(self, depot_xy, node_xy_demand, x_nodes_false, node_feature):
        ...
        if self.method == "edge_node":  
            ...
            embedded_input = self.KNN_embedding(x_nodes_false)   
            ...
            raise NotImplementedError("Unknown search method")
        out = embedded_input  # shape: (batch, problem+1, embedding)
...
class CVRP_Decoder(nn.Module):
    ...
    def forward(self, encoded_last_node, encoded_last_graph, load, ninf_mask):
        ...
        # shape: (batch, head_num, pomo, qkv_dim)
        input_cat_e = torch.cat((encoded_last_graph, load[:, :, None]), dim=2)  # shape = (batch, group, EMBEDDING_DIM+1)
